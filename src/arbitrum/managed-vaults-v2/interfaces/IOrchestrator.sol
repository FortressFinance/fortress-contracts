...
interface IOrchestrator {
    ...
    // add discription or something
    function registerVault(address _primaryToken) external returns (bytes32);

    function registerToken(address _vault, address _token) external;

    function registerStrategy() external returns (bytes32 _requestKey);

    function manageStrategy(bytes32 _requestKey) external;
