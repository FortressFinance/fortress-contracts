...
contract testFraxBPGlpConcentrator is BaseCurveConcentratorTest, InitGlpCompounder, InitFraxBPGlp {
    ...
    function setUp() public {
        ...
        tempAddr = _initializeFraxBPGlp(address(owner), address(fortressRegistry), address(fortressSwap), platform, address(glpCompounder), address(ammOperations));
...
function testDepositCap(uint256 _amount) public {
    _testDepositCap(USDC, _amount, address(payable(glpConcentrator)));
}
    ...
    function testDepositWrongAsset(uint256 _amount) public {
        _testDepositWrongAsset(_amount, WETH, address(payable(glpConcentrator)));
    }
