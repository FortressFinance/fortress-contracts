...
import "src/concentrators/balancer/AuraBalConcentrator.sol";
import "src/utils/FortressSwap.sol";
import "src/utils/FortressRegistry.sol";
import "script/utils/Addresses.sol";

import "script/utils/InitBase.sol";

contract InitWETHAURAConcentrator is InitBase {

    function _initWETHAURAConcentrator(address _fortressRegistry, address _fortressSwap, address _platform, address _compounder) public returns (address) {
        ...
        AuraBalConcentrator auraBalConcentrator = new AuraBalConcentrator(ERC20(_asset), _name, _symbol, _platform, address(_fortressSwap), _boosterPoolId, _rewardAssets2, _underlyingAssets2, _compounder);
